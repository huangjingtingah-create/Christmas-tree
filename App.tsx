...
// --- Default Photos ---
// Ensure these files exist in your public/default-photos/ folder
const DEFAULT_PHOTOS = [
    '/default-photos/01.jpg',
    '/default-photos/02.jpg',
    '/default-photos/03.jpg'
];
...
type AppMode = 'tree' | 'scatter' | 'focus' | 'wish';
...
const App: React.FC = () => {
  ...
  const modeRef = useRef<AppMode>('tree'); // Synced ref for animation loops
  ...
  const userPhotosRef = useRef<File[]>([]); // Stores accumulated user uploads
  ...
  // Sync mode changes to visuals
  useEffect(() => {
      modeRef.current = mode;
      
      // Update Morph Target on the Canvas State Ref (used by animation loop)
      if (canvasRef.current) {
          // Tree & Wish = Morph 0 (Tree Shape)
          // Scatter & Focus = Morph 1 (Scatter Shape)
          const target = (mode === 'scatter' || mode === 'focus') ? 1 : 0;
          
          // FIX: Safely initialize or update stateRef to prevent crash
          const canvasEl = canvasRef.current as any;
          if (canvasEl.stateRef) {
              canvasEl.stateRef.target = target;
          } else {
              canvasEl.stateRef = { target };
          }
      }

      // Update Focus Logic
      focusStateRef.current.active = (mode === 'focus');
  ...
  const refreshPhotos = () => {
    ...
    // 2. Combine Default and User photos
    const defaultItems = DEFAULT_PHOTOS.map(url => ({ type: 'url', src: url }));
    const userItems = userPhotosRef.current.map(file => ({ type: 'file', src: file }));
    ...
    allItems.forEach((item, idx) => {
        const loadCallback = (tex: THREE.Texture) => {
            tex.colorSpace = THREE.SRGBColorSpace;
            const geometry = new THREE.PlaneGeometry(1, 1);
            // toneMapped: false ensures vibrant colors, transparency for PNGs if any
            ...
            // Distribute evenly along the spiral based on index in combined list
            ...
            // Calculate Scatter Position (Outer Shell Only)
            ...
            const scatterPos = new THREE.Vector3(
                ...
                sr * Math.sin(sPhi) * Math.sin(sTheta) + SCATTER_CENTER_Y, 
                sr * Math.cos(sPhi)
            );

            // Initial placement depends on current mode? 
            // We just set it to treePos initially, the animation loop lerps it.
        ...
        if (item.type === 'url') {
            loader.load(item.src as string, loadCallback);
        } else {
            const url = URL.createObjectURL(item.src as File);
  ...
  useEffect(() => {
    ...
    const isMobileInitial = checkMobile();
    ...
    renderer.setPixelRatio(Math.min(window.devicePixelRatio, isMobileInitial ? 1.5 : 2));
    ...
    // FIX: Use existing stateRef if created by the other effect, or initialize it
    (canvas as any).stateRef = (canvas as any).stateRef || { target: 0 };
    ...
    // Initial Load of Default Photos
    refreshPhotos();
    ...
    const animate = () => {
      ...
      if (photos.length > 0) {
          ...
          // Focus mode scale calculation vars
          const dist = 4.0;
          const vFOV = THREE.MathUtils.degToRad(camera.fov);
          const visibleHeight = 2 * Math.tan(vFOV / 2) * dist;
          const visibleWidth = visibleHeight * camera.aspect;
          const isMob = isMobileRef.current;
          const marginW = isMob ? 0.88 : 0.6; // Wider on mobile
          const marginH = isMob ? 0.60 : 0.7; // Shorter on mobile (for UI)
          const maxW = visibleWidth * marginW;
          const maxH = visibleHeight * marginH;
          ...
          photos.forEach((p) => {
              ...
              if (p.idx === focusState.id && cameraRef.current && photoGroupRef.current) {
                  ...
                  const worldTarget = cam.position.clone().add(cam.getWorldDirection(new THREE.Vector3()).multiplyScalar(dist));
                  photoGroupRef.current.worldToLocal(worldTarget);
                  
                  // Mobile adjustment: Move slightly up to clear bottom UI
                  if (isMob) {
                      worldTarget.y += 0.2; 
                  }
                  ...
                  // Responsive Scaling
                  const scaleX = maxW / p.baseScale.x;
                  const scaleY = maxH / p.baseScale.y;
                  const finalScaleFactor = Math.min(scaleX, scaleY);
                  targetScale.multiplyScalar(finalScaleFactor);
    ...
    const handleResize = () => {
      ...
      // Clamp pixel ratio on mobile for performance
  ...
  const handlePhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      // Accumulate files, capped at 12 user photos for performance/UI safety
      // Note: Default photos do not count towards this limit.
      const updatedUserPhotos = [...userPhotosRef.current, ...newFiles].slice(0, 12);
      userPhotosRef.current = updatedUserPhotos;
      refreshPhotos();
  };
  ...
  const handleModeChange = (newMode: AppMode) => {
    manualOverrideUntilRef.current = performance.now() + 450;
    setMode(newMode);
  };
            ...
            if (ctx) {
                ...
                if (results.landmarks && results.landmarks.length > 0) {
                    ...
                    if (st.debounceCount >= 8) {
                        st.currentStableGesture = rawGesture; 
                        setDebugGesture(rawGesture);

                        // Gesture Mode Switching Logic
                        if (now > manualOverrideUntilRef.current) {
                            let targetMode: AppMode | null = null;
                            if (rawGesture === 'FIST') targetMode = 'tree';
                            else if (rawGesture === 'OPEN_PALM') targetMode = 'scatter';
                            else if (rawGesture === 'INDEX_UP') targetMode = 'focus';
                            else if (rawGesture === 'V_SIGN') targetMode = 'wish';

                            if (targetMode && targetMode !== modeRef.current) {
                                if (now - lastGestureSwitchRef.current > 700) {
                                    setMode(targetMode);
                ...
                } else { handTrackingRef.current = null; setIsHandRotating(false); }
            }
        }
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-safe right-safe pt-8 pr-8 z-50">
        <button
      ...
      <div className="absolute top-safe left-1/2 -translate-x-1/2 z-30 pointer-events-none flex flex-col items-center pt-[calc(env(safe-area-inset-top)+1rem)] md:pt-[6%]">
        <h1 className="contents">
            <a 
              href="https://www.hoenergypower.com/"
              target="_blank"
              rel="noopener noreferrer"
              className="pointer-events-auto cursor-pointer flex flex-col items-center text-lg md:text-2xl font-bold tracking-tighter text-emerald-200/90 hover:text-emerald-100 transition-colors uppercase text-center leading-[0.85]"
            >
              <span>HOENERGY</span>
      ...
      <div className={`transition-all duration-700 ${uiVisible ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        ...
        {/* 2. Interactive Gesture/Mode Bar - Bottom Centered */}
        <div className="absolute bottom-safe left-1/2 -translate-x-1/2 z-40 w-[92vw] max-w-[520px] pb-4 md:pb-8">
          <div className="flex items-center justify-start md:justify-center gap-4 bg-white/5 backdrop-blur-xl px-4 py-2 md:px-6 md:py-2 rounded-full border border-white/10 shadow-[0_4px_30px_rgba(0,0,0,0.5)] overflow-x-auto whitespace-nowrap scrollbar-hide">
              {MODES.map(item => (
                <button
                    key={item.label}
                    onClick={() => handleModeChange(item.id)}
                    className={`flex items-center gap-2 px-3 py-1.5 rounded-full transition-all duration-300 group flex-shrink-0
                      ${mode === item.id ? 'bg-emerald-500/20 shadow-[0_0_15px_rgba(16,185,129,0.3)]' : 'hover:bg-white/5'}
                    `}
                >
                  <span className="text-base md:text-lg drop-shadow-sm group-hover:scale-110 transition-transform">{item.icon}</span>
                  <span className={`text-[9px] md:text-[10px] font-bold tracking-[0.2em] uppercase transition-colors
                    ${mode === item.id ? 'text-emerald-100' : 'text-emerald-100/60 group-hover:text-emerald-100/90'}
                  `}>
                    {item.label}
                  </span>
                </button>
        ...
        {/* 3. Action Buttons - Bottom Left (Moved Up on Mobile to avoid bar overlap) */}
        <div className="absolute bottom-safe left-safe z-30 pb-24 pl-4 md:pb-8 md:pl-8">
          <button
            onClick={() => fileInputRef.current?.click()}
            className="bg-white/5 hover:bg-white/10 border border-white/10 text-white/80 hover:text-white px-6 py-2 md:px-8 md:py-3 rounded-full text-[9px] md:text-[10px] tracking-[0.2em] uppercase font-bold backdrop-blur-md transition-all shadow-xl whitespace-nowrap"
          >
            Upload 12 Photos
          </button>
        </div>

        {/* 4. Minimal Camera Preview - Bottom Right (Smaller on Mobile) */}
        <div className="absolute bottom-safe right-safe z-30 flex flex-col items-end gap-2 pb-4 pr-4 md:pb-8 md:pr-8">
           {isHandRotating && (
               <div className="text-[9px] text-emerald-400/80 font-bold tracking-[0.3em] uppercase animate-pulse">
                   Active
               </div>
           )}
           <div className="relative w-20 h-16 md:w-44 md:h-34 bg-black/40 rounded-[1rem] md:rounded-[1.5rem] overflow-hidden border border-white/5 shadow-2xl transform scale-x-[-1]">
           ...
           <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-black/20 border border-white/5 backdrop-blur-md">
              <span className={`w-1 h-1 rounded-full ${handStatus === 'ON' ? 'bg-emerald-400/60' : handStatus === 'ERROR' ? 'bg-red-400/60' : 'bg-amber-400/60 animate-pulse'}`} />
              <span className="text-[7px] md:text-[8px] text-white/40 font-bold tracking-widest uppercase">
                 {handStatus === 'ON' ? debugGesture : handStatus === 'ERROR' ? 'Disabled' : 'Syncing'}
              </span>
           </div>
        </div>
      </div>
      ...
      {mode === 'wish' && (
        <div key={wishIndex} className="absolute top-[45%] left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 animate-[fadeIn_0.5s_ease-out] pointer-events-none text-center w-full px-4">
          <h2 className="text-3xl md:text-6xl font-bold text-transparent bg-clip-text bg-gradient-to-b from-yellow-100 to-white drop-shadow-[0_0_40px_rgba(253,224,71,0.5)] mb-3 italic">
